...
export default function StockBatchForm({ medicines, onSubmit, busy }) {
  ...
  return <form onSubmit={submit} noValidate className="rounded-2xl bg-white p-7 shadow-sm"><div className="grid gap-5 sm:grid-cols-2">
    ...
    <label className="text-sm font-semibold sm:col-span-2">Source reference <span className="font-normal text-slate-500">(optional)</span><input name="sourceReference" maxLength="150" value={v.sourceReference} onChange={change} aria-invalid={Boolean(errors.sourceReference)} placeholder="Purchase order, invoice, or delivery reference" className="mt-1 w-full rounded-lg border border-slate-200 bg-[#f8fafc] p-3 focus:ring-2 focus:ring-medzo-blue" />{error('sourceReference')}</label>

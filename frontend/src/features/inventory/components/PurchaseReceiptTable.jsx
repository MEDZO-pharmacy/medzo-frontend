export default function PurchaseReceiptTable({ items = [] }) {
  ...
  return <div className="overflow-x-auto rounded-2xl bg-white shadow-sm">
    <table className="w-full text-left">
      <thead className="bg-slate-50 text-sm text-slate-500">
        <tr>{['Processed', 'Purchase reference', 'Medicine', 'Batch', 'Expiry', 'Received', 'New balance'].map(label => <th className="p-4" key={label}>{label}</th>)}</tr>
      </thead>
